...
flagFile := wf2.NewProc("flagFile", "echo 'This is a flag file' > {o:flagFile}")
targetsDirectory2 := spc.NewFileGlobberDependent(wf2, "Targets_in_Dir2", "*.json")
pythonProc3 := wf2.NewProc("pythonProc3", "python3 ../add_nonbinders_2.py {i:inpfiles2}; echo 'done' > {o:done3file}")

flagFile.SetOut("flagFile", "flag_File.txt")
targetsDirectory2.InDependency().From(flagFile.Out("flagFile"))
