...
parser.add_argument('filename', help = 'Name of the file to be balanced')
parser.add_argument('file_out', help = 'Name of the produced file')
# parser.add_argument('output_folder', help = 'Folder that contains all the targets')
args = parser.parse_args()

df_0 = pd.read_csv('database', delimiter = '\t', lineterminator = '\n', header = 0 )
# os.chdir(args.output_folder)
targ_in_name_w = os.path.basename(args.filename)   # inp filename with extention
targ_out_name_wo = os.path.basename(args.file_out) # outp filename without extention
with open(targ_in_name_w, 'r') as file:
    L0 = []
    L1 = []
    ...
    if dif < 0:
        if one_counter == 0:
            os.rename('./' + targ_in_name_w, './only_non_binders/' + targ_in_name_w)
        else:
            ...
            while nonbinders > 0:
                ...
                if df_1.iloc[k,1] == 0:
                    ...
                    L0.append('\n')
                    nonbinders -= 1
            with open(targ_out_name_wo + '_balanced.json', 'w') as outp:
        ...
        with open(targ_out_name_wo + '_balanced.json', 'w') as f:
            f.write('SMILES')
            f.write('\t')
            f.write('FLAG')
            f.write('\n')
        ...
        while dif > 0:
            ...
            if df_0.iloc[z,1] not in L_nb:
                if df_0.iloc[z,2] == 'IC50' and df_0.iloc[z,3]>10000:
                    dif += -1
                    with open(targ_out_name_wo + '_balanced.json', 'a') as f:
